...
이 저장소에서 코드로 확인할 수 있는 범위를 더 명확히 정리했습니다.

- 테스트 파일을 `src/test/java` 위치로 옮겨 테스트 코드와 운영 코드를 분리했습니다.
- 레거시 payload를 코어 도메인으로 정규화하는 mapper와 정책 판단 테스트를 정리했습니다.
- GitHub Actions CI를 추가해 테스트 실행 흐름을 남겼습니다.
- 현재 샘플의 범위와 다음 확장 방향은 [설계 샘플 정리 노트](docs/portfolio-scope.md)에 정리했습니다.
